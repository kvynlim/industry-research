...
From `50-cloud-fleet/data-platform/fleet-data-pipeline.md`, per-vehicle daily raw volumes:
...
Upload budget: 50 GB/day per vehicle (from `../../50-cloud-fleet/data-platform/cloud-backend-infrastructure.md`). This means the triage system must achieve a **5.7x data reduction** (or 82.5% discard rate) while retaining 100% of safety-critical events and maximizing the information density of what reaches the cloud.
...
Related documents:
- `50-cloud-fleet/data-platform/cloud-backend-infrastructure.md` - Receiving end (S3 ingestion, data lake, Airflow DAGs)
- `50-cloud-fleet/mlops/data-flywheel-airside.md` - Closed-loop ML from collected data
- `50-cloud-fleet/data-platform/data-engine-from-bags.md` - Rosbag batch processing for datasets
- `50-cloud-fleet/data-platform/fleet-data-pipeline.md` - End-to-end pipeline overview, DVC versioning
...
- `60-safety-validation/runtime-assurance/runtime-verification-monitoring.md` - STL monitors that generate trigger events
...
Once a clip is extracted, the edge scenario classifier assigns it a category and an annotation priority score. This metadata travels with the clip to the cloud, enabling the auto-labeling pipeline (`../../50-cloud-fleet/mlops/data-flywheel-airside.md`) to prioritize which clips get human attention first.
...
**Critical constraint**: V2X safety messages (see `30-autonomy-stack/multi-agent-v2x/v2x-protocols-airside.md`) share the same 5G network. The upload scheduler must never saturate uplink bandwidth, as this would increase latency for safety-critical V2X messages.
...
From `../../50-cloud-fleet/data-platform/cloud-backend-infrastructure.md`, cloud storage follows a tiered lifecycle:
...
**Strategy**: Record in rosbag v1 (native ROS Noetic compatibility for on-vehicle playback) and convert to MCAP during upload staging. The cloud backend (`../../50-cloud-fleet/data-platform/cloud-backend-infrastructure.md`) processes MCAP files using the standalone `mcap` library without requiring a ROS installation.
...
The on-vehicle data triage system is the first stage of the closed-loop data flywheel described in `../../50-cloud-fleet/mlops/data-flywheel-airside.md`. Its outputs directly feed the active learning pipeline:

```
┌──────────────────────────────────────────────────────────────────────────┐
│                    DATA FLYWHEEL INTEGRATION                              │
│                                                                           │
│  ON-VEHICLE (this document)       CLOUD (../../50-cloud-fleet/mlops/data-flywheel-airside.md)       │
│  ┌─────────────────────┐         ┌─────────────────────────────────┐    │
│  │ Ring Buffer          │         │                                  │    │
│  │  ↓                   │         │  Ingestion                       │    │
│  │ Trigger Detection    │         │    ↓                             │    │
...
| Cloud storage (S3 tiered) | $4-9 | From `../../50-cloud-fleet/data-platform/cloud-backend-infrastructure.md` |
...
22. `50-cloud-fleet/data-platform/cloud-backend-infrastructure.md` - Cloud receiving end
23. `50-cloud-fleet/mlops/data-flywheel-airside.md` - Closed-loop ML pipeline
24. `50-cloud-fleet/data-platform/data-engine-from-bags.md` - Rosbag processing for datasets
25. `50-cloud-fleet/data-platform/fleet-data-pipeline.md` - End-to-end pipeline overview
...
28. `60-safety-validation/runtime-assurance/runtime-verification-monitoring.md` - STL monitors as triggers
29. `20-av-platform/sensors/sensor-degradation-health-monitoring.md` - Sensor health triggers
30. `50-cloud-fleet/data-platform/fleet-data-pipeline.md` - DVC versioning and fleet data management
