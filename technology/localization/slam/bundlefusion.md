...
Do not select it as the operational airside localization stack. For real robots, use [RTAB-Map](rtab-map.md) for practical RGB-D/stereo/lidar graph SLAM experiments, or use modern lidar-inertial localization outdoors. For dense neural and Gaussian research, use BundleFusion as a quality baseline and compare against [NICE-SLAM](nice-slam.md), [Co-SLAM and ESLAM](co-slam-eslam.md), [NeRF-SLAM](nerf-slam.md), and [3D Gaussian Splatting for Real-Time AV Perception and Mapping](../../perception/gaussian-splatting-driving.md).
