...
However, production AV localization typically requires more than ORB-SLAM. It needs continuous uncertainty estimates, strict fault detection, sensor health management, deterministic integration with IMU/GNSS/wheel/LiDAR, and certified fallback behaviors. ORB-SLAM output should therefore be fused as a measurement inside the state-estimation architecture described in [Robust State Estimation and Multi-Sensor Localization Fusion](../robust-state-estimation-multi-sensor.md), rather than published directly as the vehicle's authoritative pose.
...
- Integrate with RTK-GNSS, LiDAR scan-to-map, wheel odometry, and IMU through a factor graph or ESKF, consistent with [Robust State Estimation and Multi-Sensor Localization Fusion](../robust-state-estimation-multi-sensor.md).
