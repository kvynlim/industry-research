...
EKF-SLAM is poorly matched to raw dense LiDAR point clouds, image pixels, or large unstructured maps. Those observations need front-end feature extraction before the EKF can use them. For modern LiDAR AV stacks, the LiDAR front end normally produces scan-matching odometry, loop closures, or occupancy updates rather than individual EKF landmarks; see [LiDAR Place Recognition and Re-Localization](../lidar-place-recognition-relocalization.md) and [Real-Time Occupancy Grid Mapping](../realtime-occupancy-grid-mapping.md).
...
- Validate innovations with the same monitoring philosophy used in [Robust State Estimation and Multi-Sensor Localization Fusion](../robust-state-estimation-multi-sensor.md).
...
For map construction workflows, prefer the pipeline in [Map Construction Pipeline](../map-construction-pipeline.md). For live occupancy and planning representations, use [Real-Time Occupancy Grid Mapping](../realtime-occupancy-grid-mapping.md).
