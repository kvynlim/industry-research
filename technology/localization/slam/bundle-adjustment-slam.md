...
Do not make monocular BA the primary safety-critical airside localization source. Fuse it into the multi-sensor estimator described in [Robust State Estimation and Multi-Sensor Localization Fusion](../robust-state-estimation-multi-sensor.md) and anchor it to the graph/map infrastructure in [Map Construction Pipeline](../map-construction-pipeline.md).
...
- Use LiDAR/occupancy representations for planning and safety; see [Real-Time Occupancy Grid Mapping](../realtime-occupancy-grid-mapping.md).
