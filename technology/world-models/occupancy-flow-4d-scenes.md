...
Scene flow enables tracking-by-flow as an alternative or complement to traditional detect-then-track approaches (see `../perception/multi-object-tracking.md`). Instead of detecting bounding boxes and matching them across frames, scene flow directly provides per-point motion vectors. Clustering points with similar flow vectors identifies coherent moving objects.
...
**Airside insight:** Completion is especially valuable for airside because aircraft fuselages create massive occlusion zones (30-65m blind zones, per `../perception/multi-object-tracking.md`). A completion module that hallucinates the occluded scene behind the fuselage enables the forecasting module to predict movements of ground crew or equipment hidden from direct LiDAR observation.
...
| L4 | 0.01 Hz | VLM reasoning (see `../../technology/vla/vlm-scene-understanding.md`) | 300+ ms (async) |
...
Mamba (Structured State Space Models) offers an alternative to attention for temporal modeling with **linear** complexity in sequence length (see `../../technology/perception/lidar-foundation-models.md` for Mamba in perception):
...
| Classes | 18 (see `../perception/lidar-semantic-segmentation.md`) | Airside-specific taxonomy |
...
14. **Scene flow enables tracking-by-flow as an alternative to detect-then-track.** DBSCAN clustering on (x, y, z, vx, vy, vz) identifies coherent moving objects without classification. This complements traditional MOT (see `../perception/multi-object-tracking.md`) for unknown or rare object classes.
...
- `../perception/multi-object-tracking.md` -- 3D MOT for airside
- `../perception/lidar-semantic-segmentation.md` -- 18-class airside taxonomy
- `../perception/lidar-foundation-models.md` -- PTv3, FlatFormer, Mamba backbone
