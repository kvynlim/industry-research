...
**Airside relevance:** Directly applicable to the camera fallback pipeline (see `technology/perception/camera-fallback-perception.md`). A LiDAR teacher trained on airside data can distill to a camera student for degraded-mode operation.
...
**Recommendation for airside:** ScaLR (if cameras available) or Sonata/Concerto (if LiDAR-only) for segmentation pre-training. The 18-class airside taxonomy (see `technology/perception/lidar-semantic-segmentation.md`) requires dense per-point features.
...
- `technology/perception/dinov2-foundation-models-driving.md` -- Detailed DINOv2 architecture, BEV integration, LoRA results
- `technology/perception/lidar-foundation-models.md` -- Comprehensive LiDAR foundation model survey (PTv3, Sonata, ScaLR, GPC)
- `technology/world-models/tokenized-and-jepa.md` -- JEPA paradigm, VQ-VAE tokenization, AD-L-JEPA details
- `cross-cutting/transfer-learning.md` -- Road→airside domain gap analysis, transfer strategies, UDA methods
- `technology/perception/lidar-semantic-segmentation.md` -- 18-class airside taxonomy, FlatFormer, ALPINE
...
- `technology/perception/camera-fallback-perception.md` -- Degraded mode with camera-only, BEVDistill
