...
Even with trigger-based collection (uploading only ~50 GB/day of high-value data per vehicle, as described in `50-cloud-fleet/mlops/data-flywheel-airside.md`), a 100-vehicle fleet generates **5 TB/day** of uploaded data -- $180K/year in S3 storage alone, before transfer costs.
...
| **EU PLD 2024/2853** | EU | Software/AI data governance requirements for product liability (see `80-industry-intel/regulations/regulatory-trajectory-deep-dive.md`) |
...
The data flywheel approach (detailed in `50-cloud-fleet/mlops/data-flywheel-airside.md`) is not replaced by FL -- it is complemented:
...
**Federated pre-training strategy** (extending SSL concepts from `30-autonomy-stack/perception/overview/self-supervised-pretraining-driving.md`):
...
This three-phase approach combines the data efficiency of centralized pre-training with the privacy benefits of federated fine-tuning. It aligns with the transfer learning pipeline described in `50-cloud-fleet/mlops/transfer-learning.md`, adding FL as the mechanism for multi-airport adaptation.
...
**Relevance**: In the fleet perception context (see `30-autonomy-stack/perception/overview/collaborative-fleet-perception.md`), different vehicle types produce features in different spaces. HEAL's approach -- lightweight learned projection layers that map heterogeneous features to a shared space -- applies directly to heterogeneous FL.
...
Federated continual learning (FCL) sits at the intersection of two hard problems (see `50-cloud-fleet/mlops/continual-learning.md` for the continual learning perspective):
...
- Compatible with existing data flywheel (`50-cloud-fleet/mlops/data-flywheel-airside.md`)
...
class MultiAirportFederationCoordinator:
    ...
    Integration points:
      - Data flywheel (50-cloud-fleet/mlops/data-flywheel-airside.md): FL is
      ...
      - Multi-airport adaptation (70-operations-domains/deployment-playbooks/multi-airport-adaptation.md):
...
Integration:
  - Training data from fleet data pipeline (50-cloud-fleet/data-platform/fleet-data-pipeline.md)
  - Model architecture from perception stack (30-autonomy-stack/perception/)
  - Deployment via OTA (50-cloud-fleet/ota/ota-fleet-management.md)
"""
...
9. **Byzantine-robust aggregation (FLTrust) is essential for production**: Fleet vehicles can have sensor degradation, corrupted data, or even supply-chain compromise. FLTrust requires maintaining a small trusted validation set per airport (100-500 labeled frames, provided during onboarding -- already part of the multi-airport adaptation process in `70-operations-domains/deployment-playbooks/multi-airport-adaptation.md`).
...
- `50-cloud-fleet/mlops/data-flywheel-airside.md` -- Centralized data flywheel architecture
- `50-cloud-fleet/data-platform/fleet-data-pipeline.md` -- Data infrastructure from vehicle to training cluster
- `50-cloud-fleet/mlops/transfer-learning.md` -- Road-to-airside domain adaptation
- `50-cloud-fleet/mlops/continual-learning.md` -- Continual learning fundamentals and Section 9 on FL
- `30-autonomy-stack/perception/overview/collaborative-fleet-perception.md` -- V2V cooperative perception
- `70-operations-domains/deployment-playbooks/multi-airport-adaptation.md` -- Multi-airport onboarding playbook
