...
The shift from NeRF to 3D Gaussian Splatting is also important for AV work. Gaussian maps render faster and are explicit primitives, which can make them more attractive for online perception and map maintenance. See [3D Gaussian Splatting for Real-Time AV Perception and Mapping](../../perception/overview/gaussian-splatting-driving.md).
...
For RGB-D indoor neural SLAM, start with [NICE-SLAM](nice-slam.md) and [Co-SLAM and ESLAM](co-slam-eslam.md). For practical robotics, benchmark against [RTAB-Map](rtab-map.md). For fast explicit renderable maps and AV perception integration, prioritize Gaussian methods in [3D Gaussian Splatting for Real-Time AV Perception and Mapping](../../perception/overview/gaussian-splatting-driving.md).
