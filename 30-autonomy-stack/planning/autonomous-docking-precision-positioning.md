...
- AprilTags on the GSE chassis visible to stand-mounted cameras. The camera detects the tag, computes the GSE pose, and transmits correction via V2X (see `30-autonomy-stack/multi-agent-v2x/v2x-protocols-airside.md` planned).
...
The docking controller output passes through a Control Barrier Function (CBF) safety filter before reaching actuators (see `30-autonomy-stack/planning/safety-critical-planning-cbf.md`). The CBF enforces hard constraints during docking:
...
The key architectural decision is that docking is a **mode** of the existing vehicle controller, not a separate system. When the docking state machine is active, it takes priority over the Frenet planner for velocity commands. The CBF safety filter (see `30-autonomy-stack/planning/safety-critical-planning-cbf.md`) operates on docking commands identically to driving commands -- the barrier functions simply have tighter parameters during docking mode.
