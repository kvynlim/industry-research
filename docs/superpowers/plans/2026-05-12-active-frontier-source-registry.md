# Active Frontier Source Registry Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a manual-first active frontier source registry and link it from the research loop, backlog, README, and index.

**Architecture:** The implementation is Markdown-only. The new registry page is the source/query operating manual; canonical candidate ownership stays in the existing perception audit, SLAM audit, knowledge gap backlog, and continuous research loop. No automation scripts or CI workflows are added in this plan.

**Tech Stack:** Markdown, VitePress generated navigation, existing Node 20 verification scripts.

---

## File Structure

- Create: `90-synthesis/readiness-risk/active-frontier-source-registry.md`
  - Responsibility: reader-facing source registry, query bank, candidate routing rules, and future automation boundaries.
- Modify: `90-synthesis/readiness-risk/continuous-research-loop.md`
  - Responsibility: connect the repeatable research loop to the source registry.
- Modify: `90-synthesis/readiness-risk/knowledge-gap-backlog.md`
  - Responsibility: point cross-domain gap triage back to the source registry for monitoring inputs.
- Modify: `README.md`
  - Responsibility: expose the registry from Start Here and keep public corpus counts consistent.
- Modify: `INDEX.md`
  - Responsibility: expose the registry from synthesis navigation, recently added entries, detailed file listing, and document counts.

No code files, test files, package scripts, or CI workflows are created.

---

### Task 1: Create The Registry Page
...
```powershell
Test-Path 90-synthesis\readiness-risk\active-frontier-source-registry.md
```
...
- [ ] **Step 2: Add the registry page**

Create `90-synthesis/readiness-risk/active-frontier-source-registry.md` with this content:
...
- [ ] **Step 3: Verify the registry page exists and has the required headings**

Run:

```powershell
Test-Path 90-synthesis\readiness-risk\active-frontier-source-registry.md
rg -n "^# Active Frontier Source Registry|^## Source Registry|^## Active Frontier Query Bank|^## Candidate Routing|^## Workflow And Automation" 90-synthesis\readiness-risk\active-frontier-source-registry.md
```
...
The `rg` output must include one match for each required heading.

- [ ] **Step 4: Commit the new registry page**

Run:

```powershell
git add 90-synthesis\readiness-risk\active-frontier-source-registry.md
git commit -m "docs: add active frontier source registry"
```

Expected: a commit is created with only `90-synthesis/readiness-risk/active-frontier-source-registry.md`.

---

### Task 2: Link The Registry From Existing Reader Entry Points
...
- [ ] **Step 1: Verify the new registry is not already linked from entry points**

Run:

```powershell
rg -n "active-frontier-source-registry" README.md INDEX.md 90-synthesis\readiness-risk\continuous-research-loop.md 90-synthesis\readiness-risk\knowledge-gap-backlog.md
```

Expected: no matches.

- [ ] **Step 2: Update `continuous-research-loop.md`**

In `90-synthesis/readiness-risk/continuous-research-loop.md`, after the opening paragraph and before `## Loop Contract`, add:
...
In the `## Source Audits` table, add this row before `Perception Coverage Audit`:
...
- [ ] **Step 3: Update `knowledge-gap-backlog.md`**

In `90-synthesis/readiness-risk/knowledge-gap-backlog.md`, after the first paragraph and before `## Audit Method`, add:
...
- [ ] **Step 4: Update `README.md` Start Here and counts**

In the `## Current Shape` table, change:
...
In the `## Start Here` table, add this row after `Continue the research loop`:
...
In the `## Corpus Map` table, change the `90-synthesis/` row from:
...
- [ ] **Step 5: Update `INDEX.md` synthesis navigation and counts**

In `INDEX.md`, under `#### Synthesis & strategy`, add this row after `Knowledge gap backlog`:
...
Under `## Recently Added (Latest Sessions)`, add this row at the top of the table body:
...
In the detailed file listing near the existing `continuous-research-loop.md` and `knowledge-gap-backlog.md` rows, add:
...
In the document count table, change:
...
- [ ] **Step 6: Verify all entry points link to the registry**
...
- [ ] **Step 7: Commit the cross-links and count updates**

Run:

```powershell
git add README.md INDEX.md 90-synthesis\readiness-risk\continuous-research-loop.md 90-synthesis\readiness-risk\knowledge-gap-backlog.md
git commit -m "docs: link active frontier source registry"
```

Expected: a commit is created with only the four modified Markdown entry-point files.

---

### Task 3: Verify The Public Reader
...
- [ ] **Step 3: Build the VitePress reader**
...
No output means the worktree is clean after the two documentation commits and verification.
