...
The ODD (Operational Design Domain) is defined as a machine-readable constraint set. Each parameter has four operating zones with explicit thresholds. This section complements `60-safety-validation/runtime-assurance/weather-adaptive-odd-management.md` by focusing on perception-driven ODD assessment rather than environmental weather feeds.
...
Active NOTAMs constrain the ODD spatially. See `70-operations-domains/airside/operations/ground-control-instructions.md` for NOTAM parsing.
...
The perception monitor integrates with the existing Simplex architecture (see `60-safety-validation/runtime-assurance/simplex-safety-architecture.md`):
...
- `60-safety-validation/runtime-assurance/runtime-verification-monitoring.md` -- STL temporal logic monitors, shield synthesis
- `60-safety-validation/runtime-assurance/weather-adaptive-odd-management.md` -- Environmental ODD state machine, METAR integration
...
- `60-safety-validation/verification-validation/testing-validation-methodology.md` -- Validation protocol, fault injection
